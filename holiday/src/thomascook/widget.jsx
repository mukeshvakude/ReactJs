         ...
         <div>
            <div className="container">
                ...
                <form className="d-flex" role="search">
                    <input className="form-control me-2" type="search" value={props.text} placeholder="Destination e.g. Europe / Theme e.g. Adventure" onChange={(e) => { props.setText(e.target.value); props.Search() }} aria-label="Search" />
                </form>
                                ...
                                props.searchData.destination.map((dest, idx) => {
                                    return (
                                        <ul style={mystyle}key={idx} >
                                            <li onClick={() => props.showTextInSearchBox(dest.searchString)}><a >  {dest.searchString} </a></li>
                                        </ul>
                                    )
                                })
                                ...
                                props.searchData.themes.map((theme, idx) => {
                                    return (
                                        theme.searchString !== "" && theme.searchString !== "ALL THEMES" && (
                                            <ul style={mystyle}key={idx} >
                                                <li onClick={() => props.showTextInSearchBox(theme.searchString)}><a>  {theme.searchString} </a></li>
                                            </ul>
                                        )
                                    )
                                })
                                                ...
                                                return (
                                                    holidayPackIndex >= heading.minIndex && holidayPackIndex <= heading.maxIndex && (
                                                        <ul style={mystyle}key={holidayPack.packageId + "_" + holidayPackIndex}>
                                                            <li onClick={() => props.showTextInSearchBox(holidayPack.packageName)}>{holidayPack.packageName}</li>
                                                        </ul>
                                                    ));
